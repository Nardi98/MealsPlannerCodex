...
def _bulk_plan(session):
    ...
    other = crud.create_recipe(
        session, title="Other", servings_default=1, course="main"
    )
    crud.set_meal_plan(
        session,
        {
...
def _meal(session, day):
    return session.get(Meal, (date.fromisoformat(day), 1))


def test_set_meal_plan_links_leftovers_to_source(db_session):
    _bulk_plan(db_session)

    source = _meal(db_session, D1)
    lo2 = _meal(db_session, D2)
    lo3 = _meal(db_session, D3)
...
def test_check_constraint_rejects_half_populated_link(db_session):
    db_session.add(MealPlan(plan_date=date.fromisoformat(D1)))
    db_session.flush()
...
def test_remove_leftovers_for_source_deletes_only_linked(db_session):
    _bulk_plan(db_session)

    crud.remove_leftovers_for_source(db_session, date.fromisoformat(D1), 1)
...
def test_rejecting_bulk_source_removes_its_leftovers(db_session):
    """Re-persisting the source day with a different recipe drops the leftovers."""
    _bulk, other = _bulk_plan(db_session)

    # Simulate Reject: source day is re-persisted with a different (non-bulk) recipe.
    crud.set_meal_plan(db_session, {D1: [{"main_id": other.id, "leftover": False}]})
...
def test_rejecting_a_leftover_clears_its_leftover_link(db_session):
    """Re-persisting a leftover's day as a non-leftover clears the stale link."""
    _bulk, other = _bulk_plan(db_session)

    crud.set_meal_plan(db_session, {D2: [{"main_id": other.id, "leftover": False}]})
...
def test_repersisting_a_day_preserves_unchanged_leftover_link(db_session):
    """A leftover kept on re-persist re-resolves its source from the database."""
    bulk, _other = _bulk_plan(db_session)
    ...
    crud.set_meal_plan(db_session, {D2: [{"main_id": bulk.id, "leftover": True}]})
