...
def test_generate_endpoint_debug_information(db_session):
    for i in range(3):
        crud.create_recipe(db_session, title=f"Meal {i}", servings_default=1, course="main")
    ...
    client = TestClient(app)
    response = client.post(
        "/meal-plans/generate?debug=true",
        json={"start": "2024-01-01", "days": 1, "meals_per_day": 1},
    )
    app.dependency_overrides.clear()
    assert response.status_code == 200
    data = response.json()
    assert "plan" in data and "debug" in data
    assert "2024-01-01" in data["plan"]
